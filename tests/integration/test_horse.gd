...
func test_walking_up_and_pressing_action_mounts() -> void:
	...
	assert_eq(player.controls.joypad_button_1_label.text, "Mount")
...
func test_a_horse_somebody_else_rides_refuses_the_prompt_and_the_mount() -> void:
	...
	assert_ne(player.controls.joypad_button_1_label.text, "Mount")
	player.mount(horse)
	await wait_physics_frames(3)
...
func test_getting_on_takes_the_saddle_and_getting_off_frees_it() -> void:
	...
	horse._set_rider(0) # what the server sends every peer when a rider gets off
