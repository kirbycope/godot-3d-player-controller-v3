...
func test_killing_the_duckling_brings_the_giant_boss_who_bites_and_falls_back_to_a_duckling() -> void:
	...
	assert_false(player.boss_bar.bar.visible, "The boss bar goes with the giant")
...
func test_the_giant_walks_home_and_heals_when_the_player_dies_or_runs_past_the_leash() -> void:
	...
	duck.take_hit(1000.0, player.global_position)
	await wait_physics_frames(2)
	assert_true(player.boss_bar.bar.visible, "The giant is on the Player")
	...
	assert_false(player.boss_bar.bar.visible, "The boss bar goes with the hunt")
