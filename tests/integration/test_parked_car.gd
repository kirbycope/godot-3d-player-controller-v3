...
func test_parked_car_freezes_and_wakes_for_a_driver() -> void:
	...
	await wait_until(func() -> bool: return car.freeze, 4.0)
	assert_true(car.freeze, "A settled, driverless car is frozen")
	var parked: Vector3 = car.global_position
	var crept: bool = await wait_until(func() -> bool: return car.global_position.distance_to(parked) > 0.001, 2.0)
	assert_false(crept, "The parked car does not creep")
