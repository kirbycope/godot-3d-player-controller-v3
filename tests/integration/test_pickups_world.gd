...
func test_a_dropped_item_shows_its_model_and_its_label_only_while_in_range() -> void:
	...
	assert_eq(player.controls.prompt_action_label, "Pick Up")
	player.refresh_contextual_controls()
	...
	assert_ne(player.controls.prompt_action_label, "Pick Up", "the prompt has let the label go (another prompt may be in reach there)")
	...
	assert_ne(player.controls.prompt_action_label, "Pick Up", "A pickup that vanishes in range clears the label")
