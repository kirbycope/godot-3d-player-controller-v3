...
func test_fish_index_hides_species_until_caught() -> void:
	...
	index.show_menu()
	fishing_log.record_catch(CARP, 41.0) # with the index open
	...
	assert_false(fishing_log.record_catch(BOOT, 0.0), "Junk is never a record")
	assert_true(fishing_log.has_caught(BOOT), "but it is logged as found")
