...
## round under the ProjectileSpawner and the MultiplayerSpawner replicates it, so each peer simulates the same shot. A
...
var _rounds: Array[Dictionary] = [] ## One per Projectile that entered the ProjectileSpawner: its class and its shooter.
...
func test_a_round_fired_on_the_client_flies_on_both_sides() -> void:
	...
	if is_host:
		...
		assert_null(shot, "A client's fire returns nothing; its copy arrives through the spawner")
...
## under the ProjectileSpawner as a speaker (Projectile.play_launch_sfx), which is not a round.
