...
detected 3D use, and re-running the detection would only arrive back at the same place. Only files
whose importer is "texture" are read; a WAV's .import has a compress/mode of its own that means
...
IMPORT_TEXTURE = re.compile(r'^importer="texture"$', re.MULTILINE)
...
def read_mode(text: str) -> int | None:
    """The compress/mode a texture's .import file declares, or None for anything else."""
    match = IMPORT_COMPRESS.search(text)
    return int(match.group(2)) if match and IMPORT_TEXTURE.search(text) else None
...
def fix_import_files(root: Path, dry_run: bool = False) -> tuple[int, int]:
    ...
    Three separate things can be off policy, and a file can be off on any of them. `compress/mode`
    decides how much of the image survives the import; `detect_3d/compress_to` decides whether a
    lossless texture is promoted once it is seen in 3D; `process/size_limit` decides how much of the
    image is even read, and a committed 512 there is the downscale rule wearing a different hat. The
    web build puts its own limit back on a CI checkout, which is thrown away.
    """
    fixed: int = 0
    vram: int = 0
