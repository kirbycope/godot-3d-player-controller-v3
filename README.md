...
- **Spells**: the Player's ability wheel carries Stealth, Heal and Firebolt (`resources/abilities/firebolt.tres`: a homing bolt cast forward with the TomMusic *Firebuff*, *Fireball* and *Spell Impact* sounds). Each spell's `cast_style` picks the cast clip for whatever the Player holds: sword-and-shield and greatsword casts channel in their own stance and land with their own cast or power-up clip, unarmed casts use the standing one-handed clips. Charge VFX ride the right hand and bolts leave from it. With nothing locked on, a bolt fires forward at whatever the crosshair points at, or just ahead; a lock makes it home. Spells carry element flags: Firebolt has Fire, so where it lands the grass field catches and spreads as the torch's fire does; a Water spell would douse it.
