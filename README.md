...
- **Pickups**: the weapons lying around the spawn and the fishing rod are walk-over pickups (`PlayerDetection` areas on each `Equipment` in `world.tscn`): the first time you walk over one you equip a copy and the pickup is spent, no prompt or button.
