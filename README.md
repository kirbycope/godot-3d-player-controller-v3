...
- **Horse** (`scenes/horse.tscn`, `Horse`): a rideable on the player controller's `Riding` contract, stubbed for the N-Hance horse copied from aethereal (`assets/n_hance_studio/horse/`, the brown body and saddle only). Walk up, the prompt reads Mount, Action pins the Player to the `Seat` marker (the Riding state's `seat` contract) with hands off weapons, so rider and camera turn and move with the horse in the same frame; the move input walks it, Sprint gallops at three times the walk, left and right turn it, Jump hops, Action gets off at `DismountPoint`. The Player's own camera stays the view and still looks around, with the horse's body kept out of its spring arm. The clips are in place, not root motion (no position track moves over a cycle), so the body is driven by code and the scene's `AnimationTree` only follows: a `Locomotion` blend space (turn across, pace up: `idle` at the origin, `walk` and `run` up the middle, `Walk_Backwards` below, the `Walk_Left` / `Walk_Right` and `Run_Left` / `Run_Right` cycles at the sides, synced so the gaits stay in phase) that `Horse.pace_blend()` eases toward, and a `JumpStart`, `JumpLoop`, `JumpEnd` chain whose transitions advance on the `is_jumping` flag and `is_on_floor()` through the tree's expression base node. The cycles loop through the FBX import settings (`Horse.fbx.import`), the jump start and end play once; `Swim` and friends, `Idle01`, `attack`, `hit`, `Stun` and `Death` are imported and waiting for states. The rider holds the seated `Driving` node (`rider_animation`) until a riding clip exists; the `Seat` marker is where the rider's feet go, 0.85 m up over the saddle. Drop it in any level; it is not in `world.tscn` yet.
- **Pickups**: the weapons lying around the spawn and the fishing rod are walk-over pickups (`PlayerDetection` areas on each `Equipment` in `world.tscn`): the first time you walk over one you equip a copy and the pickup is spent, no prompt or button. This is the QA world, so a spawned player is also handed a kit through `STARTING_ITEMS` in `world.gd`: 10 worms, 2 rifle clips, 1 pistol magazine and 20 arrows (`resources/items/`), topped up to those counts on every spawn. The clips, magazine and arrows are inventory items only for now; the guns and bow still count their own ammunition. Spells start at Stealth and Flash of Light with 6 skill points; the rest are on the QA spell tree (`resources/spells/qa_tree.tres`: Shadowstep behind Stealth, Consecration behind Flash of Light, Fireball and Frostbolt behind Firebolt, Lightning Bolt behind both) on the Pause menu's Spells screen, where the Loadout page arranges the eight-slot ability wheel (`scenes/world_player.tscn` sets the tree and the points on the Player's `Spellbook`).
...
| `assets/BinbunGlassUI`, `assets/BinbunMaterials`, `assets/BinbunSky`, `assets/BinbunWater`, `assets/TransitionKit` | Fluid Glass UI, Ultimate Toon Shader, Skies, Water, Modular Transitions | Binbun (Binbun3D) | not recorded - fill in | https://binbun3d.itch.io (each folder has a `.url` to its page) |
...
| `assets/gravitysound`, `addons/tcps/assets/gravitysound` (Skateboard SFX), `addons/gta/assets/gravitysound` (Car Sound Effects) | Animal SFX, Car Sound Effects, Skateboard SFX | Gravity Sound | not recorded - fill in | https://gravity-sound.itch.io/car-sound-effects, https://gravity-sound.itch.io/skateboard-sound-effects |
...
| `resources/fish/fish.svg`, `resources/fish/boot.svg`, `resources/lures/worm.svg`, `resources/lures/fly.svg` | Inventory icons for the fish and lures | Drawn for this project | CC0 | - |
...
| `assets/tommusic` | Fantasy SFX (torch loop, impacts, spell sounds) | TomMusic | not recorded - fill in (`ReadMe.txt` has no license) | https://tommusic.itch.io/ |
