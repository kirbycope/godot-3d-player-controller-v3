...
- **Fishing**: pick up the rod by the pool and press Action to cast the float where you aim (`FishingRod`, `Bobber`). Nibbles dip the float, the bite plunges it with a rumble, and one press inside the hook window hooks the fish; reeling plays out on its own and the catch arcs into your hands with a HUD card (`FishCard`). Each water's `Buoyancy` holds its fish table (`Fish` resources under `resources/fish/`, keyed by in-game hour and rain) and `FishShadows` that swim under the surface; casting near a shadow or in the rain shortens the wait. Sounds are exports on the rod, fish models are placeholders until real ones are assigned per `Fish`. The float spawns through the `ProjectileSpawner`, so other players see the float, its line (from the rod tip, or the left hand on peers without the rod), the dips and the catch.
