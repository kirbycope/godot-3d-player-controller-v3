...
- **Enemies**: four hostile mannequins east of the spawn (`EnemyNpc`, built on `FollowerNpc`): a swordsman, an archer, a rifleman and a spellcaster. Each idles until you strike it or step within five yards, then hunts you over the navmesh and attacks in reach: a sword swing, arrows and bullets that need line of sight, or abilities (`NpcCaster`: Firebolt in range with line of sight, a self-heal below half health). Hits drain your stamina and shove you; enough damage drops them into their ragdoll. Health, death and the animation state replicate from the server.
