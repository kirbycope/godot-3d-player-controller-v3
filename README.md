...
- **Spells**: each ability `.tres` (`addons/3d_player_controller/resources/abilities/`, `resources/abilities/firebolt.tres`) carries a `cast_style` plus channel, cast and impact VFX and SFX, all set in the inspector. The style picks the cast clip for whatever the Player holds: sword-and-shield and greatsword casts channel in their own stance and land with their own cast or power-up clip, unarmed casts use the standing one-handed clips. Charge VFX ride the right hand and bolts leave from it. The TomMusic spell sounds are in `addons/3d_player_controller/assets/tommusic/fantasy_sfx/Spells/`.
