...
var vortex_mesh : MeshInstance3D:
	get():
		if vortex_mesh:
			return vortex_mesh
		
		var result = get_node_or_null("Vortex")
...
@export var vortex_radius : float = 0.8:
	set(v):
		vortex_radius = v
		if vortex_mesh:
			vortex_mesh.mesh.size = Vector2(4.0 * vortex_radius, 4.0 * vortex_radius)
