...
| `assets/Flower`, `assets/Misc` | The healing flower (`flower.glb`, its texture and `flower_mesh.res`) and the `pavement_05` floor maps, both from the demo project of Le Lu's Magic Areas (Premium) pack (its `Assets/Flower` and `Assets/Misc/Floor`) | Le Lu | not recorded - fill in (Patreon pack, no license file) | https://www.patreon.com/Le_Lu/posts/godot-magic-area-102195382 (held as `D:\Gumroad\le_lu\GODOT - Magic Areas (Premium).zip`) |
| `PolyBlocks` | EffectBlocks, low poly effect blocks (the `PolyBlocks_EffectBlocks_v3` build) | bukkbeek | bukkbeek's itch.io terms: commercial use allowed, no redistribution (as catalogued in `D:\ASSETS.md`; no license file in the folder) | https://bukkbeek.itch.io/effectblocks |
...
| `assets/fonts` | FOT-Rodin Pro B, Rodin Italic | Fontworks | not recorded - fill in | Copied from the godot-zelda-breath-of-the-wild project (`D:\GitHub\godot-zelda-breath-of-the-wild\assets\fonts`); where that got them is not recorded, and `D:\ASSETS.md` does not list them |
...
| `assets/loop_box` | Zelda style light ray: `Ray_Mesh.blend` and `Shader/Line_Shader.gdshader` | loop_box | Commissioned for this owner, no license file | Commissioned directly, so it exists nowhere else; held as `D:\Commissioned\loop_box\Zelda_Ray.zip` |
...
Fetched by `tools/pull_addons.py` at the pin in `tools/addons.json`, never committed here and never pushed to.
