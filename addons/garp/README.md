...
| `Item` (`.tres` with `scripts/item.gd`) | `resources/items/` in your project | `id`, `display_name`, `description`, `icon`, `icon_color` (tints the icon in the grid, so one icon serves many variants; a subclass can override `get_icon_color()`), `category` (Equipment, Materials, Food, Key Items), `max_stack`, `consumable`, and for Equipment the `equipment_scene`. Extend the script for items with more to say: the host project's `Fish` extends `Item` with hours, rain, lures and a model. `model_scene` is a 3D scene the inventory screen turns in the details panel instead of the icon; `get_model_scene()` can fall back to a placeholder, `prepare_model(model)` dresses the instanced model (a tint, say), and `get_details(owner)` returns extra lines printed under the description, with the Player passed as `owner` (the description sits in the details panel's `DetailScroll`, a fixed area at the panel's height, so a long entry such as a fish listing ten lengths scrolls inside it instead of stretching the panel), and `get_badge(owner)` returns a short word the grid prints in the cell's top-right corner (the player controller's ammunition answers "Loaded" or "Default"); empty, the default, draws nothing. `throwable` marks a kind the game may throw (GARP only flags it; `Inventory.get_throwable_items()` lists them and `use_item(item)` uses the first stack of a kind), with `throw_damage` for what a thrown one does; `forget_equipment(item)` unequips a piece of equipment without a pickup and returns its scene path, and `add_equipment_scene(scene)` equips a fresh copy from one |
| `ItemPickup` (`scenes/item_pickup.tscn`) | In your level, on the ground | `item`, `count`, `show_icon` (the icon floats over the spot when the item has no `model_scene`; with one, the model turns on the spot under `ModelPivot`; add your own mesh and turn the icon off) |
...
**Pickups.** Standing in a pickup's `PlayerDetection` shows the `ActionPrompt` and reads the Action button as "Pick Up"; Action puts what fits in the inventory and frees the pickup once it is empty. An item with a `model_scene` lies there as that model (collision off, base on the ground, turning a full turn every `ItemPickup.TURN_SECONDS`, the same model the details panel previews) in place of the icon. The prompt and the label go down when the Player leaves the area, takes the item, or the pickup leaves the tree while they stand by it (another peer took it). Equipment items are picked up by instancing their `equipment_scene` and handing it to `equip_pickup`, exactly as a walk-over pickup does: the attach is GARP's (a `BoneAttachment3D` on the named bone holding a copy with the scene's hand offsets, world collision off, `Hitbox` kept; the instance is in the tree for the moment it equips so the offsets reach the copy; a save re-equips the same way); a second of the same type on the same bone is refused, so it stays on the ground.
