...
- `StatusBars3D` floats a health bar (green for Players, red for enemies) and a blue energy bar over the head; wire `Health.health_changed` and `Health.energy_changed` (or `Stamina.stamina_changed`) to `set_health` / `set_energy` in the scene. Bars hide while empty or full. `ProgressBar3D` is the billboarded world-space bar underneath (float values, per-instance colour, `always_visible`).
