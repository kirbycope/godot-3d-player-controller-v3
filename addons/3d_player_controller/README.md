...
- Stamina is the Player's energy; hit points live on the `Health` child (see 6c). `Player.take_hit(damage, from)` (an RPC that lands on the owning peer) costs health, shoves the Player away and rumbles the pad; `Player.heal(amount)` refills health; `register_projectile_hit` makes enemy arrows and bullets (`Projectile.damage`) count. At zero health the Player ragdolls and the `RespawnTimer` brings them back at the spawn point with full health.

### 6c. Health, Head Bars & Boss Bar (`Health`, `StatusBars3D`, `Boss`, `ProgressBar3D`)
- `Health` (`scenes/health.tscn`) is a drop-in child for any character: `max_health`, `health`, an optional `max_energy` pool with `energy_regen` on its `RegenTimer`, `damage()`, `heal()`, `spend_energy()`, and `health_changed` / `energy_changed` / `damaged` / `died` signals. Replicate `Health:health` (and energy) from the authority and puppets follow.
- `StatusBars3D` floats a red health bar and a blue energy bar over the head; wire `Health.health_changed` and `Health.energy_changed` (or `Stamina.stamina_changed`) to `set_health` / `set_energy` in the scene. Bars hide while empty or full. `ProgressBar3D` is the billboarded world-space bar underneath (float values, per-instance colour, `always_visible`).
