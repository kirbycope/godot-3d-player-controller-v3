...
- **Toon filter** (`ToonFilter`, `scenes/toon_filter.tscn`, `assets/shaders/toon_filter.gdshader`): a full-screen quad under the Player's `Camera3D` that posterises the opaque scene into `bands` luminance steps (hue and saturation kept) and draws `outline_color` lines where the depth buffer jumps, with `strength` to dial it down; a spatial shader reading only the screen and depth textures, so it compiles in Forward+, Mobile and Compatibility, draws before every CanvasLayer (HUD untouched) and before every other transparent object. Toggle with the `toggle_toon` action (`F6`) or the **Toon shading** switch in Video settings; the choice is saved as `PlayerSettingsResource.toon_enabled`. Local only, never replicated.
