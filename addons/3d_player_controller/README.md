...
- **Seeker wheel**: hold `seeker` (`I` / D-pad Up) for a second `RadialMenu` (`scenes/seeker_wheel.tscn`, `Player.seeker_wheel`). Aiming with a bow or a firearm it lists the carried ammunition kinds for that weapon and releasing on one selects it exactly as Use does (`Inventory.use_item`), so the badges and the nocked arrow follow; otherwise it lists the throwable items and releasing on one sets `Player.selected_throwable` for the next throw. It stays closed with nothing to pick, never opens on a puppet, and a carried object keeps D-pad Up for its own rotation.
