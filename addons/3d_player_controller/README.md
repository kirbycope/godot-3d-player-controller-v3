...
- Stamina doubles as the Player's health: `Player.take_hit(damage, from)` (an RPC that lands on the owning peer) drains it, shoves the Player away and rumbles the pad; `Player.heal(amount)` refills it; `register_projectile_hit` makes enemy arrows and bullets (`Projectile.damage`) count.
