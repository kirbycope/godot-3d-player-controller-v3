...
|---|---|---|
| `Inventory` (`scenes/inventory.tscn`, already inside `player.tscn`) | Child of the Player | `slots_per_tab` (20), `persist` (off by default so tests and bare Players leave the disk alone; turn it on in your game scene), `save_path` (`user://inventory.tres`) |
...
**Pickups.** Standing in a pickup's `PlayerDetection` shows the `ActionPrompt` and reads the Action button as "Pick Up"; Action puts what fits in the inventory and frees the pickup once it is empty. Equipment items are picked up by instancing their `equipment_scene` and handing it to `equip_pickup`, exactly as a walk-over pickup does: the attach is the inventory's (a `BoneAttachment3D` on the named bone holding a copy with the scene's hand offsets, world collision off, `Hitbox` kept; the instance is in the tree for the moment it equips so the offsets reach the copy; a save re-equips the same way); a second of the same type on the same bone is refused, so it stays on the ground.
